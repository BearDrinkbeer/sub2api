...
func TestMigration136AllowsWindowsADAuthProvider(t *testing.T) {
	content, err := FS.ReadFile("136_allow_windows_ad_auth_provider_type.sql")
	require.NoError(t, err)
	...
	require.Contains(t, sql, "auth_identity_channels_provider_type_check")
	require.Contains(t, sql, "pending_auth_sessions_provider_type_check")
	require.Contains(t, sql, "'windows_ad'")
}
